...
export class MessageService {
  private cryptoService: CryptoService | ExpoCryptoService;
   ...
   */
  constructor(
    cryptoService: CryptoService | ExpoCryptoService,
  ...
  async sendMessage(
    ...
    try {
      ...
      const senderId = await this.deriveSenderId(ownPublicKey);

      // Derive recipient ID from their public key
      const recipientIdHash = recipient.publicKey
        ? await this.deriveSenderId(recipient.publicKey)
  ...
  async handleIncomingMessage(
    ...
    try {
      ...
      for (const peer of peers.values()) {
        if (peer.publicKey) {
          const peerIdHash = await this.deriveSenderId(peer.publicKey);
   ...
   * @private
   */
  private async deriveSenderId(publicKey: Uint8Array): Promise<string> {
    ...
    const fingerprint = await this.cryptoService.generateTrustFingerprint(publicKey);

...
export const AppProvider: React.FC<AppProviderProps> = ({ children }) => {
  ...
  // Service instances
  const [storageService] = useState<StorageService>(
    () => new StorageService(),
  );
  const [cryptoService] = useState<CryptoService>(
    () => new CryptoService(storageService),
  );
  ...
  const handleDataReceived = useCallback(
    (deviceId: string, data: Uint8Array) => {
      ...
      messageService
        ...
        .catch(err => {
          console.error(
            '[AppContext] Failed to handle incoming message:',
            err,
          );
        });
    },
